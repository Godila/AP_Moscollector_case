...
- `moscollector.glossaryLookup` — Расшифровывает корпоративную или техническую аббревиатуру Москоллектора по справочнику сокращений. Вызывай, когда в вопросе пользователя есть непонятное сокращение или он прямо просит расшифровать аббревиатуру. Работает только по справочнику: если сокращения там нет, функция об этом сообщит, и придумывать расшифровку самостоятельно нельзя.
  Directory: functions/moscollector/glossaryLookup/
- `moscollector.navGuide` — Возвращает пошаговый сценарий входа в корпоративный сервис или перехода в его раздел: АРМ ДС, HelpDesk, DocsVision, смена пароля. Вызывай, когда пользователь спрашивает, как куда-то зайти, где что находится или как выполнить действие в интерфейсе системы. Шаги берутся из справочника сценариев: если подходящего сценария нет, функция об этом сообщит, и придумывать шаги самостоятельно нельзя.
  Directory: functions/moscollector/navGuide/
- `moscollector.publishDocument` — Сохраняет готовый документ во встроенное файловое хранилище и возвращает ссылку, по которой его можно скачать. Сама отправляет файл в чат. Вызывай, когда пользователь просит прислать документ файлом, а не текстом в переписке: служебную записку, инструкцию, перечень, график. Формат задаётся расширением в имени файла: Word, Excel или простой текст. Содержимое должно быть уже полностью составлено — функция ничего не дописывает и не проверяет.
  Directory: functions/moscollector/publishDocument/
- `moscollector.searchRegulations` — Ищет ответ в корпоративной базе знаний: регламенты, инструкции, руководства по системам. Вызывай для вопросов о порядке работ, требованиях, сроках, периодичности, действиях в нештатных ситуациях. Достаточно одного вызова: функция сама отбирает подходящие фрагменты и отсекает нерелевантные. Если вернулось found=false, повторять поиск другими словами бесполезно — этого в базе знаний нет.
  Directory: functions/moscollector/searchRegulations/
- `moscollector.draftMemo` — Готовит служебную записку по корпоративному шаблону: подбирает подходящий шаблон, проверяет состав полей по инструкции по делопроизводству и собирает текст. Вызывай, когда сотрудник просит оформить служебную или докладную записку. Если функция вернула need_fields, задай сотруднику перечисленные вопросы, дождись ответов и вызови её повторно, добавив полученные значения в fieldsJson. Придумывать значения за сотрудника нельзя.
  Directory: functions/moscollector/draftMemo/
- `moscollector.helpdeskCreate` — Оформляет заявку в HelpDeskEddy. Работает в два захода. Первый вызов ничего не создаёт: функция проверяет, все ли сведения собраны, и возвращает черновик заявки вместе с confirmToken. Покажи черновик сотруднику и спроси, создавать ли заявку. Получив согласие, вызови функцию ещё раз с теми же данными и с confirmToken из черновика — только тогда заявка будет создана. Перед созданием функция сама ищет уже открытые заявки с тем же кодом ошибки и, если такая есть, новую не заводит.
  Directory: functions/moscollector/helpdeskCreate/
- `moscollector.helpdeskStatus` — Показывает состояние заявок в HelpDeskEddy. Если сотрудник назвал номер заявки — возвращает её состояние, если нет — последние заявки. Вызывай, когда спрашивают «что с моей заявкой», «в каком статусе заявка», «какие у меня заявки».
  Directory: functions/moscollector/helpdeskStatus/
