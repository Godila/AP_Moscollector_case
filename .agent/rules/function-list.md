...
- `moscollector.helpdeskCreate` — Создаёт заявку в HelpDeskEddy. Перед созданием сама ищет уже открытые заявки с тем же признаком: если такая есть, новую не создаёт, а возвращает найденную. Вызывай только когда собраны все сведения о проблеме: в какой системе, что делали, что получилось вместо ожидаемого, код ошибки. Недостающее спрашивай у сотрудника до вызова, не придумывай за него.
